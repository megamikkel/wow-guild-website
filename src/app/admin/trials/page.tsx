...
export default async function TrialsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      <SectionHeading kicker={`${active.length} active`} title="Trials" />
      {active.length === 0 ? (
        <p className="text-ink-muted">No active trials. The bench is what it is.</p>
      ) : (
            ...
            return (
              <Surface key={t.id} className={`p-5 ${reviewDue ? "border-warn/50" : ""}`}>
                ...
                <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <dt className="stat-label">Attendance</dt>
                ...
                <p className="mt-3 text-xs text-ink-faint">
                  Started {formatDate(t.startDate)} · decision by {formatDate(t.expectedEndDate)}
                </p>
              </Surface>
            );
          })}
      ...
      <p className="mt-10 text-xs text-ink-faint">
        Data supports the decision — no algorithm decides who stays. That&apos;s an officer call.
      </p>
    </div>
  );
}
